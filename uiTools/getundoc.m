...
% 2014-10-10: removed internal mirror properties from the returned list
...
% Skip internal HG2 mirror properties - Yair 10/10/2014
validIdx = cellfun('isempty', regexp(undocumentedProps, '_I$'));
docfnames = [docfnames; undocumentedProps(~validIdx)];
undocumentedProps = undocumentedProps(validIdx);
...
%   
%   for p= fieldnames( doc )
%     undoc= rmfield( undoc , p );
%   end
%   
% 
%   if ~numel( fieldnames(undoc) )
%     undoc= [];
%   end
% 