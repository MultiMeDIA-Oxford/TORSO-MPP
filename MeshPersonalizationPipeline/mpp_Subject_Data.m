%% Subject Data
OUTPUT_FILES = {'mpp/SubjectData.txt'};
...
D = DICOMs;
for it = 1:3
  for f = sort( fieldnames( D.INFO ).' )
    fid = fopen( Fullfile( 'mpp' , 'SubjectData.txt' ) , 'a' );
    try
      s = uneval( D.INFO.(f{1}) );
      fprintf(    '%s: %s%s\n', f{1} , blanks(39-numel(f{1})) , s );
      fprintf(fid,'%s: %s%s\n', f{1} , blanks(39-numel(f{1})) , s );
      clear s;
    end
  ...
  f = fieldnames(D); f( strncmp(f,'z',1) ) = []; f( strcmp(f,'INFO') ) = [];
  D = D.(f{1});
end
