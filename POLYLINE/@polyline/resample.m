function [P,nt] = resample( P , mode , nt )
  ...
  if ~isSingle( P ), error('only single polylines can be resampled'); end
  ...
  switch lower( mode )
    ...
    case {'nn'}
      L = sum( sqrt( sum( diff( P.C{1} ,1,1).^2 ,2) ) );
      ener = @(l)nn_ener( P , l , nt , L );
      l = fzero( ener , L/nt );
      [Pr,nt] = resample( P , 'e' , l );
      if fro( Pr.C{1}(end,:) - P.C{1}(end,:) ) > 1e-10
...
function e = nn_ener( P , l , nt , L )
  Pr = resample( P , 'e' , abs(l) );
  
  e = ( size( Pr.C{1} , 1) - nt ) * L + sqrt( sum( ( Pr.C{1}(end,:) - P.C{1}(1,:) ).^2 ) );
