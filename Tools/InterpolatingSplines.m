...
D  = InterpolatingSplines( M , F , M ,'r','lambda',L);
Da = InterpolatingSplines( a*M , a*F , a*M ,'r','lambda', L/a )/a;
...
update = @()update_plus_( InterpolatingSplines( getXY(hF) , -getXY(hF) + cell2mat(arrayfun(@(h)getXY(h),hM,'un',0)) ,[] , TPS_fcn{:} ,'LAMBDA',pow10(LAMBDA.v)) );
...
%%
  ...
  if iscell( F )
    if nargout > 1, error('not implemented yet'); end
    if isempty( W ), error('not implemented yet'); end
    
    for f = 1:numel(F)
      u = InterpolatingSplines( M , F{f} , [] , G_fcn , g_fcn , varargin{:} );
      M = u(M);
      W = u(W);
    end
    uW = W;
  ...
  [varargin,~,LAMBDA] = parseargs(varargin, 'lambda','$DEFS$',Inf);
  if isnumeric( LAMBDA ) && numel( LAMBDA ) > 1
    if nargout > 1, error('not implemented yet'); end
    if isempty( W ), error('not implemented yet'); end
  
    for L = LAMBDA(:).'
      u = InterpolatingSplines( M , F , [] , G_fcn , g_fcn , 'lambda' , L , varargin{:} );
      M = u(M);
      W = u(W);
    end
    uW = W;
  ...
  if ischar( LAMBDA ) && strcmpi( LAMBDA , 'smallest' )
    LAMBDA = smallest_LAMBDA( M , F , G_fcn , g_fcn );
  elseif ischar( LAMBDA ) && strcmpi( LAMBDA , 'smallest_displacement' )
    LAMBDA = smallest_LAMBDA( M , M + F , G_fcn , g_fcn );
  else
  ...
  if ~isempty( M ) && ~isempty( F )
    w = any( ~isfinite( M ) ,2) | any( ~isfinite( F ) ,2);
    M( w ,:) = [];
    F( w ,:) = [];
  end
...
function sL = smallest_LAMBDA( M , F , varargin )
if 0
  
M = [zeros(1,2);1,0;0,0.5;1,0.5;0,1;ones(1,2)];
F = [1.71,-0.22;2.53,-0.21;-0.9,0.58;2.66,0.89;0.5,1.1;3.04,1.1]; %F(3,:) = M(3,:);
 
G = ndmat( -1:0.2:3 , -1:0.2:2 , 'lines' , -0.01 );
...
def = InterpolatingSplines( M , F , [] , 'r' );
hplot3d( def(G) , 'gray60' )


[def,bestL] = InterpolatingSplines( M , F , [] , 'r' , 'LAMBDA' , 'smallest' ); bestL = bestL.LAMBDA
hplot3d( def(G) , 'r2' )
hplot3d( def(M) , '*2k10' , 'eq');
hplot3d( M , def(M) , '2-m' );
...
hplot3d( F , 's2k10' );
hplot3d( M , F , ':r' );
hplot3d( InterpolatingSplines( M , F , G , 'r' ) , 'gray50' )


Gg_fcn = { @(x,y)ipd(x,y) , @(x)ones(size(x,1),1) };
Gg_fcn = { 'r' };

m = M;
for it = 1:500
  [~,sL] = InterpolatingSplines( m , F - m , [] , Gg_fcn{:} , 'LAMBDA' , 'smallest_displacement' ); sL.LAMBDA
  def    = InterpolatingSplines( m , F - m , [] , Gg_fcn{:} , 'LAMBDA' , sL.LAMBDA );
  m = m + def(m);
  G = G + def(G);
  hplot3d( m , '*2k10' , 'eq'); drawnow
  if maxnorm( m , F ) < 1e-8, break; end
end
...
%%  
  ...
  [~,IS] = InterpolatingSplines( M , [] , M , varargin{:} );
  R = IS.R; I = eye( size(R) );

  distance2_at0 = sum( ( M - F ).^2 ,2 );
  Y_atL = @(L) ppinv( I + R / L , F );
  distance2_atL = @(L) sum( ( Y_atL(L) - F ).^2 ,2);

  if max( distance2_at0 ) < 1e-8, sL = Inf; return; end
  if any( distance2_at0 < 1e-4 )
    thres2 = max( distance2_at0 )/( 50 ).^2;
  else
    thres2 = 1e-5;
  end
  
  
  l = fzero( @(z) max( distance2_atL( pow10(z) ) - distance2_at0 ) + thres2 , 0 );
  %figure;plotfun( @(l) max( [ distance2_atL( pow10(l) ) - distance2_at0 ] ) - thres2 , -10 , 10 , 1001); hline(0)
  sL = pow10( l );
  if sL > 1e11
    sL = Inf;
  end
