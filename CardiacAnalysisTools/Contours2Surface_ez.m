...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  allCountourPoints = [allCountourPoints; EAP];
  ...
  SCALE = max( vec( ipd( allCountourPoints( unique(round(linspace( 1 , size( allCountourPoints ,1) , 1e4 ))) ,:) , [] ) ) );
  try
    M.xyz = M.xyz/SCALE;
  catch 
      M.xyz = M.xyz/1;
  ...
  for it = 0:MAX_DEFORMATION_ITS
    ...
    if ~rem( it, min( 1e300 , FARTHESTP_RESAMPLING ) ) && it < MAX_DEFORMATION_ITS
      ...
      [~,~,ds] = vtkClosestElement( Mesh( M.xyz*SCALE , M ) , allCountourPoints );
      [~,ds] = sort( ds ); ds = ds(end-1);
      
      if ds ~= last_ds
        if FARTERPOINTS < 0
          if size( allCountourPoints ,1) < 1e5
            P = FarthestPointSampling( allCountourPoints, ds , -FARTERPOINTS , Inf );
          else
            P = GridPointSampling( allCountourPoints, ds , -FARTERPOINTS );
          end
        else
...
function M2Sub = SUBDIVIDE( M2Sub , SUBDIV_METHOD )
  ...
  switch lower( method )
    ...
    case 'fakebutterfly'
      try
        M2Sub = MeshSubdivide( M2Sub, 'butterfly' );
      catch
        M2Sub = MeshSubdivide( M2Sub, 'loop' );
      end
